...
\begin{compactitem}
\item Quantifying the development effort involved in virtualizing an accelerator API using \model.
\item Understanding \model's efficacy for different API frameworks.
\item Application performance on a \model-virtualized accelerator.
\item The sources of overhead in a virtual stack constructed with \model, and possible optimizations.
\item \model's ability to enforce resource policies (e.g., fair sharing).
\item The performance of \model's record-and-replay scheme.
\end{compactitem}
\end{comment}


\Model was evaluated on an Intel Xeon E5-2643 CPU with 128~GiB DDR4 RAM, using
...
%\Model is designed to assist virtualization engineers by automatically
...
\begin{table}
	...
	\bottomrule
	\end{tabularx}
	\caption{Development effort for forwarding different APIs. Number of APIs supported is not applicable to Python APIs as they are forwarded dynamically. The \textbf{Generated} column indicates whether the API forwarding was generated by \Compiler or was written by hand. \textbf{L.o.C.} is the number of lines of code in the \compiler specification or C/Python code. \textbf{Churn} is the total number of lines modified in commits.}
...
\compiler as evidence of the significant reduction in developer effort
\compiler provides.
We characterize developer effort to virtualize \numframeworks APIs and \numaccelerators devices in terms of the lines of code in the \compiler specifications or C/Python code (\texttt{LoC} in Table~\ref{tab:apis-and-accelerators}), and the number of lines of code modified (\textbf{Churn} in Table~\ref{tab:apis-and-accelerators}) during development (counted from commits).
...
Supporting the same subset of OpenCL with \model, took a single developer a little over a week, and the resulting API specification was 1,060 LoC long.
%The reduction in both time spent and lines of code written and modified to virtualize the OpenCL API %demonstrates that \compiler reduces the effort required to virtualize a compute accelerator API.
Even in cases where we couldn't leverage \model---%
TensorFlow and TensorFlow Lite Python APIs---%
leveraging \model's API-agnostic components enabled us to build a \novtechabbrv system with reasonable effort (3,245 lines of Python code and 2 developer weeks for TensorFlow Python).

\subsection{End-to-end Performance}

% We measured the APIs and accelerators that we virtualized with \model using the benchmarks described in Table~\ref{tab:apis-and-accelerators}.
...
\Model introduces modest overhead for most workloads.
...
\model's overheads to be amortized.
...
% \label{s:asynchrony}
% \Model allows functions annotated as
...
% \Model provides scalability and concurrency for simultaneous guest VMs and applications.
...
Accelerators without process-level protection or sharing support (e.g., Intel Movidius NCS) do not scale will with \model, as multiple applications attempting to use the device have to be serialized.
...
%\Model needs to wait for in-flight calls to finish,
