...
%\Model splits object state into two categories based on the description from the \model programmer:
...
% \Model records high-level APIs invoked by a guest.
...
%for example, \model implements a TCP channel for cross-machine migration.
...
% For non-preemptive devices and APIs, \model needs to wait for the on-going invocation to finish, which leads to unavoidable latency.
