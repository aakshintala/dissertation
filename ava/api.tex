...
\section{\Compiler and \Speclang}
\label{s:api}
\label{s:compiler}

The \Model toolchain
comprises a language (\Speclang), a compiler (\compiler), and a runtime library.
% AMP: The runtime is not written in Lapis. (It will be in Lapis 2, which I may well have mentioned at some point)
\Compiler accepts code written in \speclang (\textbf{L}ow-level \textbf{API} \textbf{S}pecification), and generates C
source for an API-specific remoting stack that implements the \novtechabbrv design.
\Speclang extends C declarations with descriptors to express a broad range of semantic information
...
% \Speclang is not inherently specialized for accelerator API remoting--the additional semantics
% required for \model are expressed in terms of more general language-level primitives:
% however, to simplify discourse, we will focus only on features relevant to \model.

To understand how \model relates to other IDL-based API-remoting techniques,
...
an IDL to specify API semantics and a compiler to automate code generation. NFS and \model share a number of
...
However, key differences between \model and NFS arise around additional virtualization requirements and
...
% system and providing compatibility at a software layer that minimizes application level change. While \model
...
% explicit offset parameters, or piggy-backing generation numbers on client file handles), \model does not have the
% luxury of changing APIs. Moreover, \model must contend with a number of additional requirements not
...
% While extending an existing IDL would provide \model with necessary semantics to deal with
...
% and API servers generated by \compiler are analogous to the stubs and skeletons created by an
% IDL compiler, \model requires a backend specialize to additionally generate device driver code, and
...
%A descriptor can also use a \speclang keyword to apply to an \spec|argument|, an \spec|element| of a buffer, or all instances of a \spec|type|.
...
%On line \ref{line:src-in} \spec|in| informs \compiler that the buffer will only be read by the call and should be copied from the application to the \worker before the call.
%The argument \spec|size| is a primitive value so \compiler can simply copy its value directly from the application to the \worker. This is the default, so no descriptor is needed.
...
\paragraphbe{Asynchrony and Implicit State.} \Speclang supports descriptors
...
%The \spec|sync| and \spec|async| descriptors specify if a call from the application can return immediately while \model handles the call asynchronously, e.g. to improve performance.
...
%  \spec|min(x, y)|/\spec|max(x, y)| & return the minimum/maximum of two values. \\
  ...
  \vspace{.1cm}
  \caption{An except from the \speclang standard library for use in expressions and utility functions.
...
\paragraphbe{State Capture.} To support VM migration, \model must capture the state of API objects and recreate that state at the destination. This requires visibility into the relationships between API calls
...
%\Model allows for zero-copy data transfer for buffers that are allocated via API function (e.g., \spec|cuHostMemAlloc|).
%However, the current implementation only supports the limited case where the API allocation functions can be replaced with \model specific implementations using the zero-copy allocation functions listed in \autoref{tab:lapis-stdlib}.
...
% \compiler-IR is similar to \speclang, but it applies a fixed set of descriptors to every value (e.g., argument) and combines all conditionals into expressions which compute specification values (e.g., the boolean representing that a function is synchronous).\amp{Is this clear?}\cjr{DISCUSS}
...
%The \speclang specification provides all the information needed for \compiler to generate serialize and deserialize the arguments since the specification provides enough information to traverse all the argument data structures.
...
The \model API-agnostic components provide shadow buffer management primitives that the generated code uses to maintain \worker-side shadows of application buffers.
...
%so \model-controlled buffers are indistinguishable from a buffer shared between the guest and the \worker, but work without shared memory and are faster than a page-level software shared memory system.
