...
We explored two solutions to support TensorFlow Python API with \model:
...
A key benefit of \model's para-virtual API-remoting approach is that the development effort required to support new APIs or devices is trivial.
A developer using \model need only be familiar with the high-level user-facing APIs,
...
\model is able to shorten the development cycle of virtualizing an API from scratch from several developer-months to a number of developer-days:

\paragraphbe{Lines of code.}
The authors built an API remoting system for a core set of OpenCL manually with X LOC in total,
though the system was much less mature compared with \model.
We rebuilt the OpenCL virtualization of the same set of APIs with \model with 850 LOC of the API specification,
and 20,000 LOC were generated automatically for the API-specific components.
We virtualized a core set of TensorFlow API manually on top of the \model's API-agnostic components,
...
However, rebuilding the system with \model cost a single developer slightly over a week,
...
%OpenCL was the first API that we tried to virtualize from the \compiler-generated guidance,
%and it exposed tens of bugs in \compiler and \model system.
%These led us to refine the specification for near twenty times,
%and improve \compiler as well as the annotation language.
We optimized \compiler and introduced new features when we supported new APIs and accelerators with \model.
\Model has been increasingly mature in the iterations,
and the annotation syntax has become more like C language's.
According to multiple \model users' feedbacks,
minimal developer intervention is now needed to virtualize an API framework with the current \model prototype,
even if they are new to \model and \compiler language:
one virtualized a core set of HIP API in about ten days.

%\Model also supports incremental change to the specification when the API is updated.
%\Compiler is able to warn any mismatches between the annotated API and latest version,
%and developers only need to update a few lines of specifications empirically.

\Model can be ported to other hypervisors and operating systems easily.
For example, \model was initially developed on Linux kernel 4.9 LTS and NVIDIA Pascal GPU cards.
...
\begin{table*}
	...
	\caption{API virtualization effort during \model development iterations. \hyu{Add it back for Chris to have a look}}
	\label{table:effort}
\end{table*}
