...
\caption{\small\uppercase{
		Comparison of existing GPU virtualization proposals, grouped
		by approach}$^{\mathrm{a}}$}
\vspace{30pt}
\scriptsize{
\resizebox{\textwidth}{!}{
...
\T\B \multirow{2}{*}{\textbf{Full-virtual}}     &
     ...
     \T\B {\textcolor{red}{0.08$\times$$^{\mathrm{c}}$}}          % virtualized speedup
     \\ \cline{2-17}
                                                &
...
\T\B
     ...
     \T\B \discrete                             &  % integrated-discrete
     \T\B Rodinia                               &  % benchmark
     \T\B 2.4$\times$                           &  % base slowdown
     \T\B 11.4$\times$                          &  % native speedup
     \T\B {\textcolor{blue}{4.8$\times$}}          % virtualized speedup
  \\ \hline
...
\vspace*{0.5pt}\xspace\\
$\mathrm{a}.$ The \textbf{\libnomod} and \textbf{\osnomod}
...
$\mathrm{b}.$ The table
includes performance entries for each system including the
geometric-mean slowdown (execution time relative to native
execution) across all reported benchmarks. %where papers
%reported sufficient details to enable us to extract this data.
We additionally include the benchmarks used, and where
possible, a report (or estimate) of the geometric-mean speedup
one should \emph{expect} for using GPUs over CPUs using
hardware similar to that used in this paper. %Where possible,
%these numbers are measured by running the benchmark suites on
%real hardware.
 The final column is the expected geometric-mean
speedup for the given benchmarks running in
the virtual GPGPU system over running on native CPUs.
The column is
computed as the expected speedup from GPUs
divided by the slowdown induced by
virtualization.

$\mathrm{c}.$ Entries where overheads eclipse GPU-based
